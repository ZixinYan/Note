...
class Network:
    ...
    def train(self, n_entries):
        ...
        for i in range(n_batches):
            batch = utils.create_data(BATCH_SIZE)
        ...
        data = utils.create_data(800)
        inputs = data[:, (0, 1)]
        outputs = self.network_forward(inputs)
        classification = classify(outputs[-1])
        data[:, 2] = classification
        utils.show(data, "After training")
...
def main():
    global current_loss
    data = utils.create_data(800)  # 生成数据
    utils.show(data, "Right classification")
    ...
    while use_this_network != 'Y' and use_this_network != 'y':
        ...
        classification = classify(outputs[-1])
        data[:, 2] = classification
        utils.show(data, "Choose network")
